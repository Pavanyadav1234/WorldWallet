...
export default function WorldWallet() {
  const [screen, setScreen]               = useState<'home'|'txs'|'swap'|'coin'>('home')
  ...
  // Auto verify on app open
  useEffect(() => {
    const timer = setTimeout(() => { doVerify() }, 600)
  ...
  const doVerify = async () => {
    ...
    try {
      ...
      if (!address) {
        setVerifyError('Could not get wallet address. Tap to retry.')
        setVerifying(false)
        return
      }
      ...
      try {
        ...
        console.error('Backend verify error:', e)
      }

      setWalletAddress(address)
      setVerified(true)
    } catch (err) {
      setVerifyError('Sign in failed. Tap to retry.')
  ...
  useEffect(() => {
    if (verified && walletAddress && (screen === 'home' || screen === 'swap' || screen === 'coin')) {
      fetchPrices()
    }
  }, [verified, walletAddress, screen, fetchPrices])

  useEffect(() => {
    if (verified && walletAddress && screen === 'txs') fetchTxs()
  }, [verified, walletAddress, screen, fetchTxs])
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      setSwapMsg(`✓ Swap submitted! Fee: ${commission.toFixed(4)} ${swapFrom}`)
    ...
    <div style={{ background:'#0B0E17', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'"Inter",system-ui,sans-serif', padding:24 }}>
      <div style={{ width:'100%', maxWidth:360, textAlign:'center' }}>
        <div style={{ position:'relative', width:100, height:100, margin:'0 auto 32px' }}>
          <div style={{ position:'absolute', inset:-10, borderRadius:'50%', background:'linear-gradient(135deg,#4F46E5,#7C3AED)', opacity:0.2, filter:'blur(24px)' }} />
        ...
        {verifying && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:16 }}>
            <div style={{ width:48, height:48, border:'3px solid #4F46E5', borderTopColor:'transparent', borderRadius:'50%', animation:'spin 1s linear infinite' }} />
            <div style={{ fontSize:15, color:'#64748B' }}>Connecting to World App...</div>
        ...
        {!verifying && verifyError && (
          <div>
            <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:14, padding:'14px 18px', marginBottom:24, fontSize:14, color:'#FCA5A5', lineHeight:1.6 }}>
              {verifyError}
            </div>
            <button onClick={doVerify} style={{ width:'100%', padding:'18px 24px', background:'linear-gradient(135deg,#4F46E5,#7C3AED)', border:'none', borderRadius:16, fontSize:16, fontWeight:700, color:'#fff', cursor:'pointer', fontFamily:'inherit', boxShadow:'0 8px 32px rgba(124,58,237,0.4)' }}>
              🌐 Try Again
            </button>
          </div>
        )}

        <div style={{ marginTop:32, display:'flex', alignItems:'center', justifyContent:'center', gap:8, fontSize:12, color:'#334155' }}>
  ...
  if (screen === 'coin' && selectedCoin) {
    ...
    const col = COIN_COLORS[sym] || '#4F46E5'
    return (
        ...
        <div style={{ maxWidth:420, margin:'0 auto', paddingBottom:90 }}>
          ...
          <div style={{ margin:'0 20px', background:'#141824', border:'1px solid #1E293B', borderRadius:18, overflow:'hidden' }}>
            {[
              ...
              {label:'24h',      val:`${up?'+':''}${selectedCoin.price_change_percentage_24h.toFixed(2)}%`, color:up?'#22C55E':'#EF4444'},
      ...
      <div style={{ maxWidth:420, margin:'0 auto', paddingBottom:90 }}>
        ...
        {screen === 'txs' && (
          <div>
            ...
            {loadingTxs ? (
              <div style={{ textAlign:'center', padding:52, color:'#475569' }}><div style={{ fontSize:36, marginBottom:14 }}>⏳</div>Loading...</div>
            ) : txs.length === 0 ? (
              <div style={{ textAlign:'center', padding:52, color:'#475569' }}>
                <div style={{ fontSize:52, marginBottom:14 }}>📭</div>
                <div style={{ fontSize:16, fontWeight:700, marginBottom:8 }}>No transactions yet</div>
                <div style={{ fontSize:14 }}>Your history will appear here</div>
              </div>
            ) : txs.map((tx, i) => {
              const isR = tx.type==='receive', isS = tx.type==='swap'
              const col  = isS?'#F59E0B':isR?'#22C55E':'#EF4444'
              const icon = isS?'⇄':isR?'↓':'↑'
              return (
        ...
        {screen === 'swap' && (
          <div>
            ...
            {swapMsg && (
              <div style={{ margin:'0 20px 16px', background:swapMsg.startsWith('✓')?'rgba(34,197,94,0.1)':'rgba(239,68,68,0.1)', border:`1px solid ${swapMsg.startsWith('✓')?'rgba(34,197,94,0.25)':'rgba(239,68,68,0.25)'}`, borderRadius:14, padding:'14px 16px', fontSize:13, fontWeight:600, color:swapMsg.startsWith('✓')?'#22C55E':'#EF4444', lineHeight:1.6 }}>{swapMsg}</div>
            )}
            ...
            <div style={{ margin:'0 20px 8px', background:'#141824', border:'1px solid #1E293B', borderRadius:20, padding:20 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <select value={swapFrom} onChange={e => setSwapFrom(e.target.value)} style={{ background:'#0B0E17', border:'1px solid #1E293B', borderRadius:12, padding:'10px 12px', color:'#F8FAFC', fontFamily:'inherit', fontSize:14, fontWeight:700, cursor:'pointer', outline:'none', flexShrink:0 }}>
                  {WORLD_CHAIN_TOKENS.map(t => <option key={t.symbol} value={t.symbol} style={{ background:'#0B0E17' }}>{t.symbol}</option>)}
                </select>
                <input type="number" placeholder="0.00" value={swapAmount} onChange={e => setSwapAmount(e.target.value)} style={{ flex:1, background:'none', border:'none', outline:'none', fontSize:28, fontWeight:800, color:'#F8FAFC', textAlign:'right', fontFamily:'inherit', minWidth:0, width:'100%' }} />
              </div>
              {fromCoin && swapAmount && (
                <div style={{ fontSize:12, color:'#475569', marginTop:8, textAlign:'right' }}>≈ ${fmt(parseFloat(swapAmount)*fromCoin.current_price)} · Bal: {fromCoin.balance} {swapFrom}</div>
              )}
            </div>

            <div style={{ textAlign:'center', padding:'10px 0', fontSize:26, color:'#7C3AED' }}>⇅</div>

            <div style={{ margin:'0 20px 20px', background:'#141824', border:'1px solid #1E293B', borderRadius:20, padding:20 }}>
              <div style={{ fontSize:11, color:'#475569', marginBottom:12, textTransform:'uppercase', letterSpacing:'0.08em' }}>To (after 10% fee)</div>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <select value={swapTo} onChange={e => setSwapTo(e.target.value)} style={{ background:'#0B0E17', border:'1px solid #1E293B', borderRadius:12, padding:'10px 12px', color:'#F8FAFC', fontFamily:'inherit', fontSize:14, fontWeight:700, cursor:'pointer', outline:'none', flexShrink:0 }}>
                  {WORLD_CHAIN_TOKENS.map(t => <option key={t.symbol} value={t.symbol} style={{ background:'#0B0E17' }}>{t.symbol}</option>)}
                </select>
                <div style={{ flex:1, textAlign:'right', fontSize:28, fontWeight:800, color:'#475569', minWidth:0, overflow:'hidden' }}>{swapOut}</div>
              </div>
              {fromCoin && toCoin && swapAmount && (
                <div style={{ fontSize:12, color:'#475569', marginTop:8, textAlign:'right' }}>1 {swapFrom} = {fmt(fromCoin.current_price/toCoin.current_price, 4)} {swapTo}</div>
              )}
            </div>

            {swapAmount && parseFloat(swapAmount) > 0 && (
              <div style={{ margin:'0 20px 20px', background:'rgba(124,58,237,0.06)', border:'1px solid rgba(124,58,237,0.18)', borderRadius:16, padding:'16px 18px' }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#7C3AED', marginBottom:10, textTransform:'uppercase', letterSpacing:'0.06em' }}>Summary</div>
                {[
                  {label:'You send',   val:`${swapAmount} ${swapFrom}`},
                  {label:'Fee (10%)',  val:`${(parseFloat(swapAmount)*0.1).toFixed(4)} ${swapFrom}`},
                  {label:'You receive',val:`${swapOut} ${swapTo}`},
                ].map((r, i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'5px 0' }}>
                    <div style={{ fontSize:13, color:'#475569' }}>{r.label}</div>
                    <div style={{ fontSize:13, fontWeight:600, color:r.label.includes('Fee')?'#F59E0B':'#94A3B8' }}>{r.val}</div>
                  </div>
                ))}
              </div>
            )}
